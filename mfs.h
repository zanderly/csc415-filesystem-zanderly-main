...
typedef struct Entry{ // declaring our entry struct
	uint64_t id;
	uint64_t location;
	uint64_t count;
	uint64_t metadata;
...
typedef struct VCB{ // declaring our VCB struct
...
typedef struct
	...
	struct fs_diriteminfo infoStruct;
	struct fs_diriteminfo* dArray;                                                        
	} fdDir;
...
Entry* createDir(uint64_t parentAddress);
//Entry *dirOpen;
